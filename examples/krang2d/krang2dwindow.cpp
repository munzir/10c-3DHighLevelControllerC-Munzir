    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->position_plot->graph(2)->setPen(QPen(Qt::green));
...
void MainWindow::update(const QVector<Scalar> &x, const QVector<Scalar> &u, const QVector<Scalar> &x_ref, const QVector<Scalar> &u_ref)
{
    ...
    ui->position_plot->graph(1)->addData(time, x[2]);
    ui->position_plot->graph(2)->addData(time, x_ref[2]);
